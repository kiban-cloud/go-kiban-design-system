...
- **Busy-region global (regla cross-module)**: cuando un trigger HTMX dispara un swap parcial, su target queda automáticamente bloqueado (gris, `pointer-events: none`) con un spinner overlay encima — feedback uniforme de "algo está pasando, no toques esto" en link/rekon/crm/klin, **sin wiring por botón**. Lo implementa `view/layout/base.templ`: la clase CSS `.kiban-busy`, los listeners `htmx:beforeRequest`/`htmx:afterRequest` (que marcan/desmarcan `evt.detail.target`), y `hx-disabled-elt="this"` heredado en `<body>` (deshabilita el propio trigger durante su request, anti doble-click). Se saltan `body`/`html` para que un swap full-page no tape la pantalla (de eso se ocupa el `nav-loader`). No requiere atributos en los componentes; basta con que el `hx-target` apunte al contenedor a bloquear.
