...
| `layout.Config` | done | Struct con Title, ProjectName, SectionLabel, User, SpaceID, ShellURL, LogoutAction, ToolKey, ActiveKey, Tools, Docs, SubItems. Cada proyecto crea un type alias local (`view_layout.PageData = layout.Config`) y un `enrich(data)` que añade los campos fijos (Tools, etc.) antes de pasar a `Layout`. **`CustomStylesheetHref string`** (opt-in): cuando no está vacío, `Base` emite un `<link rel="stylesheet">` como ÚLTIMA entrada del `<head>` para override de tokens/estilos — theming per-client (ej. el CSS por tenant del portal cliente de workfloo). El consumidor sirve el stylesheet (típicamente una ruta que streamea el CSS guardado); vacío = sin link. |
