...
| Componente | Estado | Notas |
|---|---|---|
| `table.PaginationConfig` | done | Struct con Page, HasPrev, HasNext, PageURL func(int) string, Target, Indicator, `NextVariant` (`""`/`"primary"` = solid kiban-primary "Siguiente"; `"secondary"` = mismo outlined que "Anterior" — usar en listas internas/admin donde Next no es la acción primaria). Caller construye con un closure sobre su `pageURL` local para preservar filtros entre páginas. |
| `table.Pagination(cfg)` | done | Anterior/Siguiente botones HTMX. Estados disabled-styled cuando edge. `cfg.PageURL` callback para que el caller maneje filter state. `cfg.NextVariant` opcionalmente alinea el estilo de "Siguiente" con el de "Anterior" (secondary). |
