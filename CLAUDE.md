...
| `layout.Config` | done | Struct con Title, ProjectName, SectionLabel, User, SpaceID, ShellURL, LogoutAction, ToolKey, ActiveKey, Tools, Docs, SubItems. **`TopbarExtra templ.Component`** (opt-in): se renderiza en la topbar entre el toggle de sandbox y el botón Developers; el proyecto dueño pone el markup y sus rutas (kiban-proto: su selector de fase). Nil = nada. Cada proyecto crea un type alias local (`view_layout.PageData = layout.Config`) y un `enrich(data)` que añade los campos fijos (Tools, etc.) antes de pasar a `Layout`. **`CustomStylesheetHref string`** (opt-in): cuando no está vacío, `Base` emite un `<link rel="stylesheet">` como ÚLTIMA entrada del `<head>` para override de tokens/estilos — theming per-client (ej. el CSS por tenant del portal cliente de workfloo). El consumidor sirve el stylesheet (típicamente una ruta que streamea el CSS guardado); vacío = sin link. |
...
| Tools rail toggle (slide) | done | CSS-only en `base.templ` con `.sidebar-rail-slot` (width 0 ↔ 14rem). El hamburger toggla el atributo booleano `data-sidebar-rail-open` en el root; JS persiste en `localStorage[<ProjectName>-sidebar-rail-open]` (key namespaceada por proyecto). **Sólo en desktop (≥ md):** el mismo hamburger cambia de comportamiento en móvil (ver "Mobile nav drawer"). |
| Mobile nav drawer (< md) | done | **Responsive del shell.** Debajo de `md` (768px) el grupo rail + sub-nav (`.sidebar-nav-group`, envuelto en `Layout`) sale del flujo y se vuelve un drawer off-canvas que entra desde la izquierda sobre el contenido, con backdrop (`[data-sidebar-backdrop]`). El hamburger del topbar (`[data-sidebar-show-tools]`) togglea `data-sidebar-mobile-open` en el root; se cierra por click en backdrop, click en cualquier link de nav, Escape, o al pasar a ≥ md. En el drawer el tools rail va siempre expandido y ambos rails se apilan a ancho completo. Todo el CSS del drawer está scoped a `@media (max-width: 767px)` → el shell desktop queda 100% intacto. El JS decide rail-toggle (desktop) vs drawer (móvil) con `matchMedia('(max-width: 767px)')`. El mobile-open **no** se persiste (un drawer abierto entre navegaciones sería confuso). El topbar además compacta en móvil: logo `w-24 md:w-32`, chip de espacio `max-w-[32vw]` + `min-w-0`, botón Developers `hidden md:inline-block`, divisor pre-avatar `hidden md:block`. |
| Navigation loader (`#nav-loader`) | done | Overlay full-screen mostrado al hacer click en cualquier `<a data-nav-loader>` (los items del tool rail lo llevan automáticamente) **y al navegar una fila de tabla `[data-href]`** (la delegación de click de `data-href` lo dispara antes del `window.location` — así toda navegación list→detail vía `table.Row(href,…)` muestra el loader sin wiring por fila). Cubre el wait del navegador mientras la siguiente página carga. Skip para clicks con modificadores, target=_blank, anchors `#…`, y links HTMX. Auto-hide via `pageshow` para no quedarse pegado en restores de bfcache. |
...
| `icons.Check`, `icons.Copy` | done | `Check` para confirmaciones breves ("copiado", "guardado") — el set no tenía ninguna palomita. `Copy` (dos hojas superpuestas) para copiar al portapapeles; **distinto de `Clipboard`**, que ya está tomado como glifo del conector FORM de workfloo. 20×20. |
...
| `input.Phone(label, ccName, ccValue, phoneName, phoneValue, errMsg string, required bool)` | done | Wrapper de intl-tel-input. Lleva las mismas clases de input que `Text` (borde/rounded/padding/text-sm + estado de error) y el `.iti` wrapper es `display:block; width:100%` (CSS en `base.templ`) para que se vea idéntico a los demás inputs. Renderiza `<input type="tel" data-tel-visible>` (sin `name`) + dos `<input type="hidden">` (`data-tel-cc` / `data-tel-national`). El init JS de `layout/base.templ` escanea `[data-phone-input]`. **El hidden de país (`data-tel-cc`) se emite con `+` (ej. `+52`)** — es el contrato del backend kiban (el React MF manda `+52`, y kiban-cloud/crm normalizan contra eso). `data-tel-initial` se pre-popula con `+<cc><national>` (tolera un cc ya prefijado con `+`) cuando ambos lados están seteados (edit). El input visible es **numérico-only**: `inputmode="numeric"` + `maxlength="12"` en el markup, y el `sync()` de `base.templ` sanea `visible.value` a sólo dígitos capados a 12 en cada `input`/`countrychange` (bloquea letras y >12 dígitos tanto en pantalla como en el hidden que sube al backend, alineado con la validación server `[0-9]{9,12}`). |
| `input.Code(name, label, value, errMsg string, length int, required bool)` | done | Input de código de un solo uso (NIP/OTP) de **longitud fija**: un campo centrado que sólo acepta `length` dígitos. Enforcement client-side nativo (a diferencia del resto de inputs, que dejan la validación al server): `maxlength` capa el tecleo y `pattern="\d{N}"` + HTML5 `required` bloquean el submit hasta tener exactamente N dígitos — sin JS. `length <= 0` → default 6. `inputmode="numeric"` + `autocomplete="one-time-code"` para el UX de OTP; `placeholder` muestra un bullet por dígito ("••••••"). Mismo contrato de error/label que Text (sin `hint`). Reusado por las pantallas NIP de workfloo (portal y ejecución interna). |
...
| `button.Split(cfg SplitOptions)` | done | Split button: acción primaria + caret que abre **variantes de esa misma acción** ("Guardar" / "Guardar y cerrar" / "Guardar y nuevo"). Para un menú de comandos no relacionados usá `menu.Menu`. Los items son `<button type="submit">` con `name`/`value`, así el server distingue la variante leyendo el campo posteado — sin JS propio. El dropdown reusa el MISMO handler compartido del kebab (`window.kibanToggleMenu`/`kibanCloseMenu` en `view/layout/base.templ`, keyed por `cfg.ID`), heredando outside-click, Escape, single-active y el re-anclado `fixed` que escapa contenedores con scroll. Sin `Items` degrada a un `Button` normal, así el call site puede gatear las variantes por permisos sin ramificar. `SplitItem.OnClick` corre ANTES del cierre: devolver `false` ahí aborta el submit (patrón `confirm()`). |
...
| `data-kiban-dirty-form` (atributo, no componente) | done | Marcá un `<form>` con este atributo y su submit arranca deshabilitado hasta que algún valor difiera del que llegó del servidor; volver un campo a su valor original lo deshabilita de nuevo. Deshabilita por JS (no server-side) para que un fallo del script deje el form usable en vez de bloqueado. Se re-arma en `htmx:afterSwap` con una foto nueva. Para elegir qué botón controla, marcalo con `data-kiban-dirty-submit`; sin eso agarra todo `button[type="submit"]` del form. Handler en `view/layout/base.templ`. |
| `flash.Alert(o flash.Options)` | done | Banner con acción adentro y control del cierre. `Options{Variant, Message, Dismissible, ActionLabel, ActionHref, ActionAttrs}`. **`Dismissible` es false por default**: un banner que explica por qué algo no se puede hacer no es una notificación — cerrarlo esconde la única explicación de la pantalla. El temporizador de auto-cierre sigue a `Dismissible`. Con `ActionHref` sale un `<a>`; con `ActionAttrs` y sin href, un `<button>` (acción htmx sin enlace a ninguna parte). |
...
| `menu.MenuItem` (struct) | done | El `<span>` de la etiqueta lleva `data-kiban-menu-label` para que un item que muta su propio texto (el "copiado" de un copy-to-clipboard) lo encuentre sin depender del orden del markup. `KeepOpen bool` suprime el cierre automático: para un item cuyo feedback vive DENTRO del panel (un "copiar" que se vuelve un check por un momento) — con el cierre por default el panel desaparece antes de que se vea. El item pasa a ser dueño del cierre (`window.kibanCloseMenu('<id>')`); el click-afuera y Escape siguen funcionando igual. `Label string`, `OnClick string` (raw inline JS — el menú agrega `kibanCloseMenu` al final salvo `KeepOpen`), `Variant string` (`""`/`"default"` → `text-kiban-ink`; `"danger"` → `text-red-600` para destructivos), `Icon templ.Component` opcional a la izquierda, `Attrs templ.Attributes` para HTMX/data-* (mismo patrón que `button.Options.Attrs`). |
...
| `tabs.Strip(items []TabItem, activeKey string)` | done | Renderiza el strip con el visual contract descrito arriba. Active state es 100% caller-driven: si ningún `Key` matchea `activeKey`, no se resalta nada. Múltiples instancias en la misma página son seguras (cada `<a>` lleva su propio `href`/Attrs). **Responsive:** el contenedor lleva `overflow-x-auto` y cada tab `shrink-0 whitespace-nowrap`, así un strip con muchos tabs scrollea horizontalmente en móvil en vez de desbordar la página (verificado con 8 tabs a 375px; el `-mb-px` del baseline no genera scroll vertical). |
